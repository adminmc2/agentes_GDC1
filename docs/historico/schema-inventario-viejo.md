# Schema del inventario — VERSIÓN ANTIGUA (pre-rediseño)

> ⚠️ **NO USAR. VERSIÓN HISTÓRICA preservada para referencia (v1 pre-rediseño 2026-05-12).**
> El schema canónico actual vive en `schema-inventario.md`. Esta versión describe el modelo viejo (3 bloques `vocabulario_consolidado` con `comprension`, campos `contenido_linguistico` y `campo_semantico` aún presentes). Se conserva mientras `validar_inventario.py` siga validando contra este shape (será reescrito en Paso 3 del plan global de rediseño).
