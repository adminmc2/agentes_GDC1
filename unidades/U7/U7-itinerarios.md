# Itinerario — Unidad 7: Hábitos

## ITINERARIO BÁSICO

### SECCIÓN VOCABULARIO {color: salmón}

- **[CLASE] EJERCICIOS 1-2** Los meses del año y las estaciones
  Orden de los doce meses con la pista 72 y escritura de los meses de cada estación en el propio país.
- **[CLASE] EJERCICIOS 3-4** Cumpleaños y fechas señaladas
  El calendario de cumpleaños de la clase y la escucha de las fechas de las fiestas con la pista 73 y la tarjeta de escucha.
- **[CLASE] EJERCICIO 5** Presentación del vocabulario de los animales
  Escucha y repetición de los once animales de las fotos con la pista 74.
- **[CASA] EJERCICIO 6** Clasificación de los once animales en domésticos y salvajes en la tabla
- **[CLASE] EJERCICIO 7** Los animales de la granja
  Identificación de los cuatro animales de la granja con la pista 75 y juego con las Tarjetas de Vocabulario - animales.
- **[CLASE] EJERCICIO 8** El primer día del potro Rayo
  Lectura del texto con la pista 76, las seis preguntas del test y su corrección en plenaria.

### SECCIÓN GRAMÁTICA {color: azul}

- **[CLASE] EJERCICIOS 1-2** El presente de los verbos reflexivos
  El cuadro de los verbos reflexivos de la página 74, práctica guiada en los dos ejercicios y el juego de adivinar el personaje.
- **[CASA] EJERCICIO 3** Descripción de lo que hace la persona de cada foto siguiendo el ejemplo del libro
- **[CLASE] EJERCICIOS 4-5** Las preposiciones *a*, *en*, *de* y *por* {píldora formativa 7.1 · gramática · el mapa del día de Lucía}
  El mapa del día de Lucía en la pizarra, el cuadro de usos de la página 75 y práctica en los dos ejercicios.
- **[CLASE] EJERCICIOS 6-7** Los verbos *salir* y *volver*
  El cuadro de *salir* y *volver* de la página 75, los infinitivos de cada forma y la construcción de las siete frases.
- **[CLASE] EJERCICIOS 8-9** Los signos de puntuación
  Reconocimiento de los signos y de su nombre en plenaria y colocación de los que faltan en las seis frases.

### SECCIÓN COMUNICACIÓN {color: verde}

- **[CLASE] EJERCICIOS 1-3** Vídeo del sábado de Pablo y Jorge
  Visionado del vídeo con la pista 77, completado del diálogo y comprensión con verdadero o falso y frases por ordenar.
- **[CLASE] EJERCICIOS 4-6** La rutina por horas con la tarjeta *Preguntar y decir cómo es tu día*
  Preguntas por turnos con los verbos del recuadro, entrevista a alguien del centro y las siete viñetas en parejas.
- **[CASA] EJERCICIO 7** Texto breve sobre la rutina de la pareja empezando por la fórmula del libro
- **[CLASE] EJERCICIOS 8-10** La rutina de Enrique y el chat de los domingos
  Horarios de Enrique con la pista 78, contraste con las predicciones de la clase y chat sobre los domingos en parejas.
- **[CLASE] EJERCICIOS 11-13** La entonación de preguntas y exclamaciones
  Reconocimiento y repetición de la entonación con la pista 79 y colocación de los signos según lo que se oye con la pista 80.

### SECCIÓN DESTREZAS {color: amarillo}

- **[CLASE] EJERCICIOS 1-2** El día de Pedro, cuidador del zoo de Madrid {píldora formativa 7.2 · destreza · leer un texto descriptivo}
  Lectura del texto por sus marcadores temporales, la línea del día en la pizarra y la clasificación de los animales del texto.
- **[CASA] EJERCICIO 3** Completado de las cinco frases sobre el día de Pedro releyendo el texto
- **[CLASE] EJERCICIO 4** Reto de memoria sobre el día de Pedro
  Reconstrucción de memoria de la línea del día y completado de las cuatro frases con las palabras subrayadas.
- **[CLASE] EJERCICIO 5** La historia de Budi el orangután
  Escucha de la historia de Budi con la pista 81, verdadero o falso y corrección de las falsas en parejas.
- **[CLASE] EJERCICIOS 6-8** La mascota imaginaria y la visita al zoo con la tarjeta *Presenta en tres pasos*
  Escritura de la mascota imaginaria, discusión en grupos sobre los animales y concurso de propuestas de visita al zoo.

### SECCIÓN CULTURA Y EVALUACIÓN {color: naranja}

- **[CLASE] EJERCICIOS 1-2** El Zoo-Aquarium de Madrid
  Preguntas al zoo antes de leer, lectura con la pista 82, comprensión del texto y contraste con un sitio del propio entorno.
- **EVALUACIÓN — Versión A** Seis ejercicios como deberes + revisión en plenaria
- **EVALUACIÓN — Versión B** Sesión completa: realización escalonada en tres tandas + corrección colectiva

---

## ITINERARIO EXTENDIDO

### ESTACIÓN DE SERVICIO

Imprime estas tarjetas al principio de la unidad y podrán usarlas de forma individualizada tus estudiantes y facilitarte la atención diferenciada y cercana.

**CAJA 1—VOCABULARIO** — 29 tarjetas
- 29 {animales}

**CAJA 2—ESTRATEGIAS** — 2 tarjetas
- 1 {comunicación}
- 1 {expresión oral}

### Materiales complementarios

#### Vocabulario {color: salmón}

- **BLINKLEARNING Juega y aprende** (Ej. 1-3) — 3 ejercicios interactivos: orden de los meses del año (1), sopa de letras de animales (2) y audios con la imagen del animal (3)
- **Actividades Extra 7** *Días, meses, estaciones* — Ejercicios 1-4 (15-20 min)
- **Actividades Extra 7** *Animales* — Ejercicios 5-7 (20-30 min)
- **Gamificación** *Calendario indiscreto* — Ejercicios 1-4 (50 min)

#### Gramática {color: azul}

- **BLINKLEARNING Juega y aprende** (Ej. 4-5) — 2 ejercicios interactivos: los verbos reflexivos por su pronombre (4) y la elección de pronombre y preposición (5)
- **Actividades Extra 7** *Hábitos* — Ejercicios 1-3 (20-30 min)
- **Gamificación** *Dibujo encadenado* — Ejercicios 1-3 (30-40 min)

#### Comunicación {color: verde}

- **Vídeo Blinklearning** — Ejercicios 1-3 (5-10 min)
- **Power Point Unidad 7** *Hablar de horarios* — Ejercicios 4-6 (10-20 min)
- **Actividades Extra 7** *Entrevistas* — Ejercicio 5 (25-35 min)

#### Destrezas {color: amarillo}

- **Actividades Extra 7** *El día perfecto de…* — Ejercicios 6-8 (25-35 min)

#### Cultura y Evaluación {color: naranja}

- **Evaluación 7** — (40-50 min)
- **Vídeo final** *La vida de Marcos* — Unidades 6 y 7 (30-40 min)

---

## Notas para extracción de imágenes (diseñador)

Origen físico de cada material complementario para extraer portadas o miniaturas en la maqueta InDesign del itinerario extendido. Orden coincidente con el orden de aparición en el extendido.

#### Vocabulario

| # | Material | Archivo fuente | Páginas (extensión) | Imagen para maqueta (página exacta) |
|---|---|---|---|---|
| 1 | BLINKLEARNING Juega y aprende — Ej. 1-3 | capturas depositadas: `ej1.png`, `ej2.png`, `ej3.png` | — | captura del Ej. 2 (la sopa de letras con los ocho animales fotografiados, `ej2.png`) |
| 2 | Actividades Extra 7 *Días, meses, estaciones* | `Compa4ed1ActividadesextraU7.pdf` | pp. 1-2 | p. 2 (ficha del estudiante con las quince frases y el recuadro de meses) |
| 3 | Actividades Extra 7 *Animales* | `Compa4ed1ActividadesextraU7.pdf` | pp. 3-4 | p. 4 (ficha con los dos grupos de animales dibujados —12 domésticos y 10 salvajes— y el cartón de bingo) |
| 4 | Gamificación *Calendario indiscreto* | `Compa4ed1_Cuadernodejuegos.pdf` | pp. 40-41 del cuaderno | p. 41 (hoja recortable *Mi calendario* con los doce meses ilustrados) |

#### Gramática

| # | Material | Archivo fuente | Páginas (extensión) | Imagen para maqueta (página exacta) |
|---|---|---|---|---|
| 1 | BLINKLEARNING Juega y aprende — Ej. 4-5 | capturas depositadas: `ej4.png`, `ej5.png` | — | captura del Ej. 4 (las cinco cajas de pronombres *me/te/se/nos/os* con los diez verbos, `ej4.png`) |
| 2 | Actividades Extra 7 *Hábitos* | `Compa4ed1ActividadesextraU7.pdf` | pp. 5-6 | p. 6 (fichas de Alumno A y Alumno B con la tabla *Tú / Tu compañero*) |
| 3 | Gamificación *Dibujo encadenado* | `Compa4ed1_Cuadernodejuegos.pdf` | p. 39 del cuaderno | p. 39 (cabecera con el ejemplo del papel doblado *Monto en bicicleta*) |

#### Comunicación

| # | Material | Archivo fuente | Páginas (extensión) | Imagen para maqueta (página exacta) |
|---|---|---|---|---|
| 1 | Vídeo Blinklearning | `video1.png` (fotograma depositado; pendiente: verificación de título y enlace) | — | fotograma de la conversación telefónica de Pablo y Jorge a pantalla partida (`video1.png`) |
| 2 | Power Point Unidad 7 *Hablar de horarios* | `Compa4ed1_UNIDAD7.pptx` | 8 diapositivas | diapositiva 5 (el reloj con las fracciones de la hora, *y cinco… menos cuarto*) |
| 3 | Actividades Extra 7 *Entrevistas* | `Compa4ed1ActividadesextraU7.pdf` | pp. 9-10 | p. 10 (transcripción de la entrevista a Juan con los once huecos) |

#### Destrezas

| # | Material | Archivo fuente | Páginas (extensión) | Imagen para maqueta (página exacta) |
|---|---|---|---|---|
| 1 | Actividades Extra 7 *El día perfecto de…* | `Compa4ed1ActividadesextraU7.pdf` | pp. 7-8 | p. 8 (hoja recortable con las diez tarjetas de personajes) |

#### Cultura y Evaluación

| # | Material | Archivo fuente | Páginas (extensión) | Imagen para maqueta (página exacta) |
|---|---|---|---|---|
| 1 | Evaluación 7 | `Compa4ed1EvaluacionU7.pdf` | pp. 1-2 | p. 1 (cabecera con el bloque *Vocabulario* y las seis fotos de animales) |
| 2 | Vídeo final *La vida de Marcos* con su ficha | `Compa4ed1_FichaVideosU6-7.pdf` + `video2.png` (fotograma depositado) | 1 p. (cubre las unidades 6 y 7) | fotograma del desayuno de Marcos con su padre (`video2.png`) |
