...
**Cómo se detectan los candidatos (v12.24: ampliado a cuatro fuentes).** La detección automática y la detección editorial **conviven en la misma cola** `propuestas[]` con el mismo gate humano de cierre. El payload `relacion_candidata` (schema §6) discrimina la fuente con el enum cerrado `fuente_deteccion`:

- **`cuadro_compartido`** — fuente automática mecánica. Helper `scripts/proponer_relaciones_cuadro.py`: para cada par de hilos que comparten ≥1 cuadro en la misma unidad, crea/actualiza propuesta con `evidencia.referencias = [cuadros]` (prefijo `cuadro@`). Un solo cuadro tratando dos contenidos es indicio fuerte de relación editorial. `razonamiento` opcional.
- **`actividad`** — fuente editorial. La sesión Capa 2 detecta que dos hilos coocurren funcionalmente en actividades atestadas sin compartir cuadro (p. ej. `Adjetivos de nacionalidad` ↔ `verb·ser` en p15-act3 "Yo soy español"). `evidencia.referencias` lleva patrones `p<N>-act<M>[@R]`; `razonamiento` obligatorio (1-3 frases que justifican el vínculo funcional).
- **`indice_curso`** — fuente editorial. La sesión Capa 2 detecta que el índice del curso agrupa dos hilos en una sola entrada (p. ej. "Verbos ser, llamarse y tener" en U1 gramatica agrupa tres hilos verbales). `evidencia.referencias` lleva formato `U<N>:<campo>:<entrada>` (formato estructural verificado por el validador; la **existencia literal** de la entrada en `nc1-curso.json` queda como responsabilidad editorial de la sesión, no estructural — chequeo cross-archivo diferido); `razonamiento` obligatorio.
- **`encuadre_editorial`** — fuente editorial residual. Conexión léxico-gramatical razonada que no encaja en las tres anteriores. `evidencia.referencias` admite mix (cuadros + actividades + entradas); `razonamiento` obligatorio.

**Contrato mínimo de evidencia para fuentes editoriales.** Una propuesta de fuente editorial (no-cuadro) **debe** traer al menos una referencia material atestada del corpus + razonamiento no vacío. El validador chequea no-vacío; el objetivo editorial son 1-3 frases por razonamiento. Sin evidencia material atestada, no es candidato — es invento.

**Política de cierre: proposal first uniforme.** Las cuatro fuentes pasan por el mismo gate humano antes de escribir entrada en `hilo.relaciones[]`. Nada se canoniza directamente. La distinción cuadro / editorial vive en `fuente_deteccion`, no en la política de cierre.

**Principio rector (Nivel 5, v12.19).** Hoy: IA + autor (proposal first; cierre humano para las cuatro fuentes). La arquitectura permite transición gradual a direct-write por sub-tipo cuando una fuente esté lo suficientemente estabilizada como para que el cierre directo no sea apuesta (probable orden de estabilización: `cuadro_compartido` → `indice_curso` → `actividad` / `encuadre_editorial`).
