...
### Nivel 2 — Contrato operativo — ✅ COMPLETO (2026-05-21)

El contrato operativo de fase 2 está escrito al estándar de fase 1: contrato corto (`CLAUDE.md`) + schema (`schema-reciclaje.md`) + reglas (`reglas-reciclaje.md`, con §12 ciclo de vida y §13 validación) + prompt envoltorio (`prompt.md`).
...
- ✅ **Prompt envoltorio de fase 2** — `fases/2-reciclaje/prompt.md`, creado en v11.49. Entry point operativo por unidad: gate de arranque, input/output, flujo Capa 1/Capa 2, criterio de cierre. Esqueleto-contrato: el detalle del pipeline es Nivel 3.
...
2. ✅ **Matriz de decisión para `etiquetas[]`** (v12.21). Cerrada en `reglas-reciclaje.md` §3.1 con 6 predicados binarios deterministas (A, B, C, D_any, D_ant, E) sobre el canónico + proyección canónica de fase 1 (los 5 registries — vocab/gram/pron-orto/verbal/perífrasis, con perífrasis decidida por el registry `perifrasis-canonicas.json`). Dos capas: Capa 1 asigna función principal entre `introduce`/`amplia`/`aplica`/`sistematiza`/`anticipacion`; Capa 2 admite tres coexistencias documentadas (`introduce+sistematiza`, `amplia+sistematiza`, `aplica+anticipacion`). Excepción explícita conservada: distinción `amplia` vs `sistematiza` cuando ambos cumplen requisitos, único resto de juicio editorial. Hilos sin canónico fuera de matriz. `contrasta` y `discrimina` quedan fuera del cierre determinista por falta de input estructurado en el inventario — formalizado en §3-bis con condiciones de reapertura. La matriz reproduce el juicio editorial real de los 89 eventos etiquetados en U0-U3.
3. ✅ **Ampliación de §15 — detección cross-hilo multifuente** (v12.24). Cerrada en `reglas-reciclaje.md` §15 + `schema-reciclaje.md` §6. La detección de candidatos `relacion_cross_hilo` deja de ser exclusivamente cuadro-bound: el payload `relacion_candidata` (schema §6) incorpora enum cerrado `fuente_deteccion` con 4 valores (`cuadro_compartido` automático mecánico + 3 fuentes editoriales: `actividad`, `indice_curso`, `encuadre_editorial`) y objeto `evidencia` (referencias con formato cerrado por fuente + razonamiento obligatorio para fuentes editoriales). Todas las fuentes pasan por el mismo gate humano antes de escribir entrada en `hilo.relaciones[]` (política proposal first uniforme). Helper `proponer_relaciones_cuadro.py` y validador `validar_reciclaje.py` actualizados al nuevo shape; el validador gana canal de avisos (separado de errores) que señala propuestas legacy pendientes de migrar. Reemplaza el sub-punto 3b del plan anterior (checklist de cierre): el contrato mínimo de evidencia y la política de cierre uniforme cubren su función operativa. Sub-punto 3a (criterio verbal `procedencia_indice`) queda desacoplado y se aborda como lote propio (v12.26 en la nueva numeración).
4. **Marco lingüístico-didáctico del mapa de relaciones.** Documento de fundamentación que define cómo se identifica una relación, basado en categorías reconocidas de didáctica ELE y lingüística aplicada (relaciones léxico-gramaticales, requisitos pragmáticos, dependencias morfosintácticas, contraste paradigmático, anclaje funcional-comunicativo). Revisa la taxonomía actual de 5 tipos (`usa`/`prerrequisito`/`activa`/`contrasta`/`comparte`) a la luz del marco y decide si bastan, sobran o faltan tipos. Soporte teórico del trabajo posterior.
5. **Rúbrica fija de `explicacion`.** Decide forma del campo `analisis_ia`: rúbrica de 5 bloques (Identidad / Estado previo / Estado en esta unidad / Función didáctica / Conexiones cross-hilo). Decide también si `explicacion` se queda cuadro-bound o se amplía a "toda relación requiere explicación, con o sin cuadro". Toca schema §4 + reglas §7 + prompt. Prerrequisito del piloto: sin rúbrica fija, el piloto produce texto heterogéneo.
6. **3a verbal rediseñado.** Criterio operativo para `procedencia_indice` en bloque `verbal` (§4 reglas), restringido a respaldo en `gramatica` del índice del curso (no `comunicacion`/`cultura`/`destrezas`/`para_aprender`/`pronunciacion_ortografia`). Word boundary match (no substring crudo). Fallback `pcic:A1` para lemas en `verbos-canonicos.json` sin entrada en `gramatica` del índice. Validación contra casos U0-U3 + casos críticos (`hablar`, `hacer` deben quedar `pcic:A1`, no reconciliados por rótulos comunicativos).
7. **Piloto en U4.** Primera unidad bajo contrato completo (§3.1 etiquetas + §3 verbal + §15 ampliada + rúbrica `explicacion`). Shakedown de la procedimentalización.
8. **Visualización del mapa de relaciones en dashboard.** Post-piloto, no bloqueante del arranque U4. Visualiza el mapa con sus relaciones, tipos y evidencia para que el autor revise sin abrir JSON. Lote independiente; puede pausarse o reanudarse sin afectar el cierre del contrato.
9. **Retrofit U0-U3.** Auditar y, si procede, ajustar las sesiones previas contra el contrato cerrado. Después de validar el piloto, no antes.
10. **Consolidación del mapa al cierre del libro.** `fases/2-reciclaje/mapa-relaciones-A1.json` — **registro derivado versionado de relaciones canónicas por nivel**. Artefacto distinto de los cinco registries del universo de hilos (§6) — esos definen identidades; este define arcos entre identidades. Mismas convenciones de versionado y `_meta`. Construcción incremental durante U4-U9 + consolidación al cierre. Script de validación que verifica coherencia, completitud y alias resueltos. Reduce significativamente la necesidad de IA por unidad en libros A1 posteriores (consumen el mapa en lugar de construirlo).
11. **(Diferido)** `scripts/enriquecer_capa2.py` — solo si tras estabilizar con U4 + retrofit + mapa consolidado las heurísticas se demuestran lo suficientemente cerradas para mecanizar parcialmente. Congelar heurísticas antes de formalizarlas sería prematuro.
