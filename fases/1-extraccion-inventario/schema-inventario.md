...
- Sufijo opcional `@R` — solo válido en fuentes de actividad. Indica que la palabra aparece únicamente en el campo `respuestas` de una actividad de **producción** (5 tipos: `produccion_escrita_guiada`, `expresion_escrita_libre`, `expresion_oral_libre`, `tarea_final`, `interaccion_oral`). Indica output producido por el alumno, no input leído del libro.
...
- **Lista de tipos productivos que admiten sufijo `@R` (§9.5)** — los 5 tipos canónicos de la taxonomía cerrada §5 son `produccion_escrita_guiada`, `expresion_escrita_libre`, `expresion_oral_libre`, `tarea_final`, `interaccion_oral`. La regla operativa que decide cuándo aplicar `@R` debe usar exactamente esta lista. Cualquier divergencia es bug.
