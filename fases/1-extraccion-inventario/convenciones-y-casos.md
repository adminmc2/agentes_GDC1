...
**"delante" codificado como léxico de U2 desde una instrucción de aula (v10.144).** Detectado tras revisión del autor durante consolidación de U5: el bloque `Marcadores de lugar: a la izquierda, ..., cerca` aparecía en U5 como "recurrente desde U2", pero al inspeccionar U2 las únicas apariciones reales eran `izquierda/derecha` como claves técnicas de `columnas_relaciona` (metadata interna) y `delante` en *"Representa el diálogo delante de la clase"* — instrucción de aula del libro al alumno, no contenido didáctico. La regla §5.2 de exclusión por metalengua de instrucción solo cubría verbos en su redacción anterior, no léxico, por lo que `delante` se filtró como ref léxica en U2 y arrastró el falso "recurrente" a U5. **Corrección estructural (v10.144):** §5.2 generalizada a las 4 dimensiones (léxico, verbos, gramática, pron/orto). **Corrección material:** en U5, bloque `Adverbios de posición` retirado y `Marcadores de lugar` reclasificado de recurrente → principal (primera aparición real en NC1, no recurrente). U2 queda sin codificación de marcadores espaciales (correcto: no los enseña). Aprendizaje para futuras extracciones: las palabras que aparecen únicamente en `instruccion_original` o en títulos metalingüísticos de cuadros (*"Observa"*, *"Para aprender"*, etc.) no codifican refs ni en léxico ni en ninguna otra dimensión.
