...
5. **Schema documental ↔ validador no divergen como principio canónico** — `schema-inventario.md` y `scripts/validar_inventario.py` son contratos paralelos; cualquier divergencia es bug a resolver. La deuda residual (parte editorial no automatizable: §5.10 Categoría B + excepciones léxicas de §5.11) ya no invalida el gate automático de cierre y queda delimitada en el Apéndice transitorio (§A.3).
