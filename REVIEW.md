...
- **2026-05-14** — Reset de `reglas-operativas.md` bajo modelo IA-first (v10.115). Hito de cierre del lote de rediseño operativo de fase 1. Archivo reescrito desde cero como autoridad operativa única (§0 política transversal, §1-§10 cuerpo, banner reducido a 2 follow-ups vivos). Familia `@R` realineada a 5 tipos productivos (no 6) en reglas/schema/glosario. §5.6 acotado a léxico con exclusiones explícitas para los 3 bloques no-léxicos. `reglas-operativas-viejo.md` congelado como reservorio histórico (header de archivo, follow-ups marcados CERRADOS con punteros al vivo, boilerplate neutralizado). Schema §A.3 depurado del falso pendiente de Regla 11. `prompt.md` y `CLAUDE.md` de fase 1 saneados para reflejar la deuda transitoria de `convenciones-y-casos.md` (consulta acotada al `-viejo` con no-prevalencia sobre schema/reglas). Verificación atómica de refs `§X.Y` cruzadas: cero rotas. Lote siguiente pendiente: reset de `convenciones-y-casos.md`.
